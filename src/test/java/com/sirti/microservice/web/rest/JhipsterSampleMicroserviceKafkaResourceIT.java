package com.sirti.microservice.web.rest;
...
import com.sirti.microservice.IntegrationTest;
import com.sirti.microservice.config.EmbeddedKafka;
import com.sirti.microservice.config.KafkaSseConsumer;
import com.sirti.microservice.config.KafkaSseProducer;
...
class JhipsterSampleMicroserviceKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/jhipster-sample-microservice-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/jhipster-sample-microservice-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/jhipster-sample-microservice-kafka/unregister"));
                return;
            }
        }
